...
const OUTPUT_DIR = path.join(__dirname, "../cumples-semana");
const OUTPUT_FILE = path.join(OUTPUT_DIR, "cumples-semana" + formatted + ".png");
...
fs.createReadStream("cumples.csv")
  ...
  .on("end", async () => {
    try {
      ...
      let hoy;
      if (args.length >= 2) {
        // Si se pasan día y mes, usar esos valores
        const diaArg = parseInt(args[0], 10);
        const mesArg = parseInt(args[1], 10);
        // Año actual
        const year = dayjs().year();
        hoy = dayjs(`${year}-${mesArg}-${diaArg}`);
      } else {
      ...
      const background = await loadImage(path.join(__dirname, "imagenes/background.png"));
      ...
      for (let i = 0; i < cumpleaneros.length; i++) {
        ...
        // Carga de Fotos
        let fotoNombre = emp.PhotoName && emp.PhotoName.trim() ? emp.PhotoName.trim() : "user.png";
        ...
        try {
          ...
          console.log(
            `❌ Error al cargar la imagen ${emp.PhotoName}:`,
            err.message
          );
          continue;
        }

        const burbuja = await loadImage(path.join(__dirname, "imagenes/day-bubble.png"));
        ...
        const nombre = emp.FirstName.trim() + " " + emp.LastName.trim();
